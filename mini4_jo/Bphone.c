...
int loadCSV(struct PHONE_RECORD *phonebook, int size){
	...
	int k = 0;
	while(k < size && fgets(array, 999, p) != NULL) {
...
int addRecord(struct PHONE_RECORD *phonebook, char nameInput[], char birthdayInput[], char phoneInput[], int size) {
	int i = 0;
	int filled = 0;
	while(i < size) {
		if (phonebook[i].name[0] == '\0') {
...
int listRecords(struct PHONE_RECORD *phonebook, int size) {
