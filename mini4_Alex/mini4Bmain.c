...
int main(){
    ...
    printf("Desired phonebook size:");
    fgets(size_char,11,stdin);
    
    int size = atoi(size_char);

    struct PHONE_RECORD *p = makeArray(size);
    

    int x = loadCSV(size);
    if(x==2){
        printf("Array too big! Terminating program. \n");
        return 1;
    } 
...
//          while((c = getc(stdin)) != '\n' && c != EOF); // To remove any newline character from the stdin buffer;
            ...
            addRecord(name, birth, phone, size); 
...
//          while((c = getc(stdin)) != '\n' && c != EOF); // to remove any newline character from the stdin buffer
        ...
        case 3:
            ...
            if(getSize( size)<1){ // means the file doesnt exist
                printf("The file \"phonebook.csv\" is empty.\n");
                break;
            }
